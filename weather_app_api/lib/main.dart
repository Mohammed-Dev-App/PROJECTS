...
import 'package:weather_app_api/widget/weather_data_tile.dart';
...
class _WeatherPageState extends State<WeatherPage> {
  ...
  getData(String cityName) async {
    ...
    var weatherData;
    if (_cityName == '') {
      weatherData = await weatherService.fetchWeather(cityName);
    } else {
  ...
  Future<bool> checkLocationPermisson() async {
    LocationPermission _permisson = await Geolocator.checkPermission();
    if (_permisson == LocationPermission.denied) {
      _permisson = await Geolocator.requestPermission();
      if (_permisson == LocationPermission.denied) {
        return false;
      }
      getData('');
